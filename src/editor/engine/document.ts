...
function warpTextCanvas(doc: PsDocument, base: HTMLCanvasElement, spec: TextSpec): HTMLCanvasElement {
  ...
  for (let ix = 0; ix < width; ix++) {
    ...
    if (style === 'arc') {
      ...
      yShift = -amp * (1 - Math.abs(u))
    } else if (style === 'bulge') {
      scaleY = Math.max(.12, 1 + bend * (1 - u * u) * .85)
    } else if (style === 'flag') {
      yShift = amp * Math.sin(t * Math.PI * 2)
    } else if (style === 'wave') {
      yShift = amp * Math.sin(t * Math.PI * 4)
    }
  ...
  const cx = b.x + b.w / 2
  const by = b.y + b.h / 2
  // Photoshop's horizontal/vertical distortion acts like an editable
  // perspective skew. Keep the range conservative to avoid singular matrices.
  const shx = hDist * .45
  const shy = vDist * .45
  oc.translate(cx, by)
  oc.transform(1, shy, shx, 1, 0, 0)
  oc.translate(-cx, -by)
